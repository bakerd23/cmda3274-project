...
After gathering the HDI stats for the men, we trained a random forest model on the data to predict each player's rating from per-game and efficiency statistics: on points per game (ppg), rebounds per game (rpg), assists per game (apg), steals per game (spg), blocks per game (bpg), 2 point shot percentage (two_pct), 3 point shot percentage (three_pct), free throw percentage (ft_pct), turnovers per game (tov), assist to turnover ratio (a_to), effective field goal percentage (efg), true shooting percentage (ts), and free throw rate (ftr). We then applied this model to the men's and women's data to assign each player a "statistics score".
