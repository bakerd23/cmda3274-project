...
Figure 1 illustrates the four-step pipeline used in this project: data collection, cleaning, modeling, and conference adjustment.
...
```
```{r, include = FALSE}
kable(head(mbb_raw_printing,5), caption = "Table 2. Example of raw men's model data")
```
```{r, include=FALSE}
...
```
```{r, include=FALSE}
kable(head(model_data_printing,5), caption = "Table 1. Example of cleaned men's model data")
```
