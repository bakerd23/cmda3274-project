...
1. HDI ratings & player statistics, accessed through a Google Sheet provided by Virginia Tech Men's Basketball staff and downloaded as a CSV.
2. Barttorvik statistics, scraped using the `cbbdata` package (Weatherman, 2024) for the corresponding seasons.
The two datasets were joined using a custom matching function designed to handle inconsistencies in naming and duplicate names across teams. Players appearing in only one dataset or with incomplete records were removed.
